...
func NewRedis(addr string, keyPairs ...[]byte) (Store, error) {
...
func NewRedisSecure(host, port, user, password string, db int, keyPairs ...[]byte) (Store, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*30)
	defer cancel()
...
func (s *store) Options(options sessions.Options) {
	s.SessionOptions = options.ToGorillaOptions()
}
...
func (s *store) load(session *gsessions.Session) (bool, error) {
...
func (s *store) delete(session *gsessions.Session) error {
