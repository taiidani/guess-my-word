...
test('reads current state', async ({ page }) => {
  ...
  await expect(page.locator('#stats-yesterday')).toContainText(defaultYesterday)
  await expect(page.locator('.before')).toHaveText('apple')
  await expect(page.locator('.after')).toHaveText('No guesses after the word')
})
...
test('ignores prior state', async ({ page }) => {
  ...
  await expect(page.locator('#stats-yesterday')).toContainText(defaultToday)
  await expect(page.locator('.before')).toHaveText('No guesses before the word')
  await expect(page.locator('.after')).toHaveText('No guesses after the word')
})
